...
// 顺序查找
function sequentialSearch(values: number[], value: number): number {
    let result = -1;
...
// 二分查找： 返回坐标
function binarySearch(values: number[], value: number): number {
    let result = -1;
    if (values?.length > 0) {
...
function binarySearchLoop(values: number[], value: number): number {
    let result = -1;
    if (values?.length > 0) {
        let count = values.length;
...
export function main() {
    let array1 = [1, 3, 4, 8, 9, 12];
    let searched = 4;
    let result = binarySearch(array1, searched);
